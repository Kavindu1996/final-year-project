    ...
    if (isset($_GET['user_id'])) {
        ...
        if ($result_set) {
            if (mysqli_num_rows($result_set) == 1) {
                ...
                header('Location: staff.php?user_not_found');
            }
        }else {
            header('Location: staff.php?query_failed');
        }
    }
           ...
           if ($_FILES['stud_image']['size']< 200000) {
                ...
                if ($img_width <= "1000" && $img_height <= "1500") {
                        if (empty($errors)) {
                            ...
                            //encrytion the password
                            $hashed_password    = sha1($password);

                            //query
                            $query = "UPDATE staff SET first_name = '{$first_name}', last_name = '{$last_name}', phone_number = '{$phone_number}', email = '{$email}', date_of_birth = '{$date_of_birth}', nic = '{$nic}', address = '{$address}', gender = '{$gender}',  role_id = '{$role_id}', stud_image = '{$stud_image}', password = '{$hashed_password}', employee_id = '{$employee_id}' WHERE id = {$user_id} LIMIT 1";
                            //query execute function
                            $result = mysqli_query($connection, $query);

                                    //if query execute correctly
                                    if ($result) {
                                            move_uploaded_file($_FILES["stud_image"]["tmp_name"], "../upload/staff/" . $stud_image);
...
<head>
    ...
    <link rel="stylesheet" href="CSS/modify-staffProfileStyle3.css">
</head>
<body>
...
<div class="container">
        <div class="title">
            <div class="routing">Staff Mmebers / Modify Staff Registration</div>
            <b>Modify Staff Registration</b>
        </div>
            <div class="mobiletitle">
            <b>Modify Staff Registration</b>
            </div>
            <div class="mobilerouting">Staff Mmebers / Modify Staff Registration</div>
    <!--validation display-->
        <?php
                ...
                <div class="input_box_fifth">
                    <div class="fifthLeft">
                        ...
                        <div class="inputColumnFifthLeft">
                            <!-- <input id="inputImage" type="file"  accept ="image/" onchange="loadfile(event)"  name="stud_image"  >
                    ...
                    <?php
                        $query = "SELECT * FROM role WHERE role_name != 'admin'";
                        $result = mysqli_query($connection, $query);
